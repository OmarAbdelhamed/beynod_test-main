...
import { Alert, Modal, StyleSheet, Text, FlatList, View, Image, Dimensions, TouchableOpacity, Animated, ScrollView } from 'react-native';
...
import HomeOfWoman from '../Components/HomeOfWoman';
import { useDispatch, useSelector } from 'react-redux';
import { useNavigation } from '@react-navigation/native';
import { addUser, authInfo } from '../Redux/Authantication';
...
const HomeScreen = () => {
    ...
    const getMainCategoury = () => {
        axios.get('https://beyond-fix.applaiteknoloji.online/api/get_main_categories')
            .then((response) => {
                // Handle the response
            ...
            const getUserData = () => {
                axios.get('https://beyond-fix.applaiteknoloji.online/api/profile',{
                    headers:{
...
{data && data.map((item, key) => {
                    ...
                    <TouchableOpacity
                                onPress={() => setCatType('Best Seller')}
                            >
                                <Text
                                    style={{
                                        ...
                                        fontSize: 16,
                                        color: 'rgba(0, 0, 0, 1)',
                                        textDecorationLine: catType === 'Best Seller' ? 'underline' : 'none'
                                    }}
                                >Best Seller</Text>
                            </TouchableOpacity>
                            {mainCategories && mainCategories.map((cat, index) => {
                                return (
                                    <TouchableOpacity
                                        onPress={() =>{ setCatType(cat.name.en)
                                            setCatID(cat.id)
                                    ...
                                    >
                                        <Text
                                            style={{
                                                ...
                                                fontSize: 16,
                                                color: 'rgba(0, 0, 0, 1)',
                                                textDecorationLine: catType === cat.name.en ? 'underline' : 'none'
                                            }}
                                        >
                                            {cat.name.en}
                                        </Text>
